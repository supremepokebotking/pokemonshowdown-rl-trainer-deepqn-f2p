...
class PokeDuelingDeepQNetworkUriah(nn.Module):
    def __init__(self, lr, n_actions, name, input_dims, chkpt_dir):
        super(PokeDuelingDeepQNetworkUriah, self).__init__()
    ...
    def forward(self, full_obs):
        ...
        action_mask = full_obs[:,-self.n_actions:]
        #print('len  ction_mask', len(action_mask))
        #print('cccaction_mask', action_mask)
...
#        conv = self.max_2(conv)
        ...
        # Pass the output through the output layer and mask based on the legal actions
        #output_layer_output = nn.functional.softmax(self.output_layer(hidden_layer_output), dim=1)
        masked_output_layer_output = torch.where(action_mask == 1, returnlogs, torch.tensor(float("-Inf")).to(self.device))
        #masked_output_layer_output = returnlogs * action_mask.float()

        V = returnvals
        A = masked_output_layer_output
