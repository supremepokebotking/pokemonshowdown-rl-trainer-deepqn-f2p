 ...
 *********************************************************/
 ...
 const Punishments = new (_class = class {
	...
	async lock(
		...
		for (const curUser of affected) {
			exports.Punishments.setPunishmentTimer(curUser, expireTime, punishment[1]);
			curUser.locked = punishment[1];
	...
	automaticUnlock(user, punishedID) {
	 ...
	 */
	setPunishmentTimer(user, expireTime, punishedID) {
		if (expireTime === 0 || expireTime === Infinity || expireTime > Chat.MAX_TIMEOUT_DURATION) return;
		user.punishmentTimer = setTimeout(exports.Punishments.automaticUnlock, expireTime - Date.now(), user, punishedID);
	}
	async namelock(
		user, expireTime, id, ignoreAlts, ...reason
	) {
	...
	checkName(user, userid, registered) {
		...
		exports.Punishments.setPunishmentTimer(user, punishment[2], punishment[1]);
	}
